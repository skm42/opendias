...
int main (int argc, char **argv) {
  ...
  if( config_setting == NULL || config_value == NULL ) {
	o_log(INFORMATION,"running testExec");
	char *imageData=NULL;
	char *progressData=NULL;

	size_t imageSize,progressSize;

	o_exec(SYNC,&imageData,&progressData,&imageSize,&progressSize,1,"/usr/bin/scanimage","-p");
	//o_exec(SYNC,0,"/home/computer/projects/openDIAS/tmp/genOutput");

	o_log(INFORMATION,"imageDate size %d, progressData size %d",(int)imageSize,(int)progressSize);
	o_log(DEBUGM,"imageDate content(%s)",imageData);
	
	int scan_fd;
	if ( (scan_fd=open("/tmp/test.pnm",O_CREAT|O_WRONLY,S_IWUSR|S_IRUSR|S_IRGRP|S_IWGRP)) == -1 ) {
		printf("cannot create scan file");
		exit(0);
	}
	if ( write(scan_fd,imageData,imageSize) == -1 ) {
		o_log(ERROR,"imageDate write failed %s",strerror(errno));
		exit(1);
	}
	close(scan_fd);

	o_log(INFORMATION,"running testExec completed");
    close_all();
    return 0;
  }
